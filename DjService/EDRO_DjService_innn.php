...
class EDRO_DjService
	...
	private function _CreateListen_lnSock()
		...
		if($bIzSocket)
			...
			{
			$this->arrReport['socket_conect']='Failed';
			}
		/*if($bIzBinded)
			{
			echo 'socket_bind: OK'."\n";
	...
	private function _SocketListen_strAudioBuffer()
		...
		//	{
			$this->_CreateListen_lnSock();
			echo socket_write($this->lnSOCK, "GET /pulstranceHD.mp3 HTTP/1.1\r\n\r\n");
			echo "\n";
			while(true)
				{
				echo $strSockData=socket_read($this->lnSOCK, 4096, PHP_BINARY_READ);
				//file_put_contents('t.mpX', $strSockData, FILE_APPEND);
				}
